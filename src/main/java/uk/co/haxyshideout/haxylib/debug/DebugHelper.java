...
public class DebugHelper
{
	...
	{// Check missing block lang entries
		...
		{
			// <3 if (GameRegistry.findBlock(modID, block.getUnlocalizedName()) != null)// Only check blocks for pixelmon
			// <3	continue;
			
			//GameRegistry.findRegistry(registryType)
			if (GameRegistry.findRegistry()findBlock(modID, block.getUnlocalizedName()) != null)// Only check blocks for pixelmon
				continue;
