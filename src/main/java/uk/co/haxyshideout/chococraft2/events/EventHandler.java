...
public class EventHandler
{
	...
	public void onConnect(PlayerEvent.PlayerLoggedInEvent event)
	{
		...
		if (checkResult.status == ForgeVersion.Status.OUTDATED)
		{
			...
			Style clickableChatStyle = new Style().setClickEvent(clickEvent);
			event.player.sendMessage(new TextComponentString(ChatFormatting.GOLD + "Chococraft 2 Update available!, Click here to open website").setStyle(clickableChatStyle));
			event.player.sendMessage(new TextComponentString(checkResult.changes.get(checkResult.target)).setStyle(new Style().setColor(TextFormatting.AQUA)));
			/* <3 Style clickableChatStyle = new Style().setChatClickEvent(clickEvent);
	...
	public void onRightClickChocopedia(PlayerInteractEvent event) throws IOException
	{
		...
		if (event instanceof PlayerInteractEvent.RightClickEmpty)
		{
			if (event.getEntityPlayer().getHeldItem(event.getHand()) != null && event.getEntityPlayer().getHeldItem(event.getHand()).getItem() == Additions.chocopediaItem)
			{
				...
				// <3 ConfigurationNode configurationLoader = HoconConfigurationLoader.builder().setSource(CharSource.wrap(new String(data))).build().load();
